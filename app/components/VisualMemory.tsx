  ...
  if (phase === "ready") {
    return (
      <div className="text-center">
        <button
          onClick={startGame}
          className="px-8 py-4 bg-pink-600 text-ink font-bold text-xl rounded-2xl hover:bg-pink-700 transition-colors"
        >
          Start Visual Memory
        </button>
  ...
  if (phase === "correct") {
    return (
      <div className="text-center space-y-4">
        ...
        <button
          onClick={nextLevel}
          className="px-8 py-3 bg-pink-600 text-ink font-bold rounded-xl hover:bg-pink-700 transition-colors"
        >
    ...
    return (
      <div className="text-center space-y-6">
        <div className="bg-paper-2 rounded-2xl p-8 border border-line">
          <p className="text-ink-2 text-sm mb-2">Visual Memory Score</p>
          <p className="text-6xl font-black text-pink-400">Level {score}</p>
          <p className="text-ink-2 mt-1">{tilesForLevel(score)} tiles memorized</p>
    ...
    <div className="space-y-4">
      ...
      >
        {Array.from({ length: total }).map((_, idx) => {
          ...
          let bg = "bg-paper-2 hover:bg-paper-2 cursor-pointer";
      ...
      {phase === "show" && (
        <p className="text-center text-ink-3 text-sm">Memorize the pattern...</p>
      )}
    </div>
  );
}
